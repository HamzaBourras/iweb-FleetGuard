...
export default function Dashboard() {
  ...
  // ✨ UI/UX : Classe dynamique pour centrer l'icône si le menu est réduit
  const navLinkClasses = ({ isActive }) =>
    `relative group flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3 rounded-xl font-medium transition-all duration-300 ease-out ${isActive
      ? "bg-blue-500/10 text-blue-400 border border-blue-500/20 shadow-[inset_0_1px_0_0_rgba(59,130,246,0.1)]"
      : `text-slate-400 hover:bg-slate-800/80 hover:text-white ${!isSidebarCollapsed ? 'hover:translate-x-1' : ''}`
    }`;
  ...
  return (
    <div className="h-screen bg-slate-50 flex font-sans selection:bg-blue-200 overflow-hidden">
      ...
      {/* ✨ NOUVEAU : Largeur dynamique (w-72 ou md:w-20) */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 bg-[#0B1120] text-white flex flex-col justify-between p-5 shadow-2xl border-r border-slate-800/50 transition-all duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} ${isSidebarCollapsed ? 'md:w-20 w-72' : 'w-72'}`}
      >
        ...
        <div className="relative z-10">
          {/* ✨ NOUVEAU : Masquage du logo si le menu est réduit */}
          <div className={`pb-4 mt-4 md:mt-0 flex justify-center transition-all duration-300 ${isSidebarCollapsed ? 'opacity-0 h-0' : 'opacity-100 h-auto'}`}>
            <img
              src={logoImg}
          ...
          {/* ✨ NOUVEAU : Logo minimaliste (Icône) quand le menu est réduit */}
          <div className={`pb-4 mt-4 md:mt-0 flex justify-center transition-all duration-300 ${!isSidebarCollapsed ? 'hidden' : 'block'}`}>
            <ShieldCheck className="w-8 h-8 text-blue-500" />
          </div>

          <div className={`flex flex-col items-center border-b border-slate-100 pb-4 mb-6 transition-all duration-300 ${isSidebarCollapsed ? 'opacity-0 h-0 p-0 mb-2 border-transparent' : 'opacity-100 h-auto'}`}>
          ...
          <nav className="space-y-3 mt-4">
            ...
            <NavLink to="/dashboard" end className={navLinkClasses} onClick={closeMobileMenu}>
              {({ isActive }) => (
                <>
                  <LayoutDashboard className={`w-5 h-5 transition-transform duration-300 shrink-0 ${isActive ? 'scale-110 text-blue-400' : 'group-hover:scale-110 text-slate-500 group-hover:text-white'}`} />
                  {!isSidebarCollapsed && <span className="whitespace-nowrap transition-opacity duration-300">Vue d'ensemble</span>}
                  
                  {/* ✨ TOOLTIP AU SURVOL (Sidebar réduite) */}
                  {isSidebarCollapsed && (
                    <div className="absolute left-full ml-3 px-3 py-1.5 bg-slate-800 text-slate-100 text-xs font-semibold rounded-md shadow-lg opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all pointer-events-none z-50 whitespace-nowrap">
                      Vue d'ensemble
                    </div>
                  )}
                </>
            ...
            <NavLink to="/dashboard/sites" className={navLinkClasses} onClick={closeMobileMenu}>
              {({ isActive }) => (
                <>
                  <Globe className={`w-5 h-5 transition-transform duration-300 shrink-0 ${isActive ? 'scale-110 text-blue-400' : 'group-hover:scale-110 text-slate-500 group-hover:text-white'}`} />
                  {!isSidebarCollapsed && <span className="whitespace-nowrap transition-opacity duration-300">Sites WordPress</span>}
            ...
            <NavLink to="/dashboard/alerts" className={navLinkClasses} onClick={closeMobileMenu}>
              {({ isActive }) => (
                <>
                  <ShieldAlert className={`w-5 h-5 transition-transform duration-300 shrink-0 ${isActive ? 'scale-110 text-blue-400' : 'group-hover:scale-110 text-slate-500 group-hover:text-white'}`} />
                  {!isSidebarCollapsed && <span className="whitespace-nowrap transition-opacity duration-300">Alertes de Sécurité</span>}
            ...
            <NavLink to="/dashboard/guide" className={navLinkClasses} onClick={closeMobileMenu}>
              {({ isActive }) => (
                <>
                  <BookOpen className={`w-5 h-5 transition-transform duration-300 shrink-0 ${isActive ? 'scale-110 text-blue-400' : 'group-hover:scale-110 text-slate-500 group-hover:text-white'}`} />
                  {!isSidebarCollapsed && <span className="whitespace-nowrap transition-opacity duration-300">Documentation SOC</span>}
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative w-full">
        ...
        <header className="bg-white/80 backdrop-blur-md shadow-[0_1px_2px_0_rgba(0,0,0,0.03)] h-[72px] flex items-center justify-between px-4 md:px-8 border-b border-slate-200 shrink-0 z-10">
          <div className="flex items-center gap-3">
            ...
            {/* ✨ NOUVEAU : Bouton pour réduire le menu (Desktop) */}
            <button
              onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
              className="hidden md:flex p-2 -ml-2 mr-2 text-slate-600 hover:bg-slate-100 hover:text-blue-600 rounded-lg transition-colors focus:outline-none"
